    ...
    <div className="relative my-2">
      ...
      >
        ...
        <div className="flex-1 flex flex-col justify-center pr-2 py-1 space-y-1">
          <h2
            translate="no"
            className="font-extrabold text-lg sm:text-xl text-white tracking-wide uppercase leading-snug"
          >
            {descripcion}
          </h2>

          <p className="text-primary text-xl sm:text-2xl font-extrabold">
            ${precio.toFixed(0)}
          </p>

          {observaciones && (
            <p className="text-gray-300 text-xs sm:text-sm font-normal line-clamp-2">
              {observaciones}
            </p>
          )}
          ...
          {seccion?.nombre === "EMPANADAS" && (
            <div className="flex gap-2 pt-2" onClick={(e) => e.stopPropagation()}>
              <button
                onClick={addProducto}
                className="bg-white cursor-pointer hover:bg-primary hover:text-white text-primary font-bold text-xs px-3 py-1 rounded-full shadow transition-all"
              >
                x1
              </button>
              <button
                onClick={addProductoPor6}
                className="bg-white cursor-pointer hover:bg-primary hover:text-white text-primary font-bold text-xs px-3 py-1 rounded-full shadow transition-all"
              >
                x6
              </button>
              <button
                onClick={addProductoPor12}
                className="bg-white hover:bg-primary hover:text-white cursor-pointer text-primary font-bold text-xs px-3 py-1 rounded-full shadow transition-all"
              >
                x12
              </button>
            </div>
        ...
        <button 
          ...
          disabled={sinStock}
          className="w-full bg-primary hover:bg-primary/90 active:scale-[0.99] text-white font-bold py-2.5 px-4 rounded-[1.2rem] flex items-center justify-center gap-2 transition-all shadow-md hover:shadow-primary/30"
        >
